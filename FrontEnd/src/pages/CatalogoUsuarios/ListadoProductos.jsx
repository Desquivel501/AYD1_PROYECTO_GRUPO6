...
export default function ListadoProductos() {
  ...
  useEffect(() => {
    postData({ endpoint: "ObtenerCombos", data: { correo: datos.correo } })
      .then((response) => {
        setCombo(response);
      });

    postData({ endpoint: "ObtenerProductos", data: { correo: datos.correo } })
      .then((response) => {
  ...
  const addCarrito = (p_id, name, tipo, cant, costo, img) => {
    ...
    for (var i = 0; i < carrito.productos.length; i++) {
      if (
        carrito.productos[i].id == p_id && carrito.productos[i].tipo == tipo
      ) {
